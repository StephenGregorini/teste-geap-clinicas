...
export default function App() {
  ...
  // Listas
  const [listaOperadoras] = useState(["GEAP", "AMIL", "TODOS"]);
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      if (nomeClinica.length > 2) {
        ...
        if (data) {
          setNameSuggestions([
            ...new Set(data.map((c) => c.nome_fantasia)),
          ]);
        }
      } else {
        setNameSuggestions([]);
    ...
    async function loadInitialFilters() {
      ...
      if (especialidadesData) {
        const uniqueEspecialidades = [
          ...new Set(
            especialidadesData.map((x) => x.especialidade?.trim())
          ),
        ].filter(Boolean);
      ...
      if (ufsData) {
        const uniqueUFs = [
          ...new Set(ufsData.map((x) => x.uf?.trim())),
        ].filter(Boolean);
  ...
  useEffect(() => {
    async function carregarCidadesPorUF(u) {
      ...
      const unique = [
        ...new Set(data.map((x) => x.cidade?.trim())),
      ].filter(Boolean);
  ...
  async function buscar() {
    ...
    if (operadora !== "TODOS") q = q.eq("operadora", operadora);
    ...
    data.forEach((c) => {
      ...
      if (c.especialidade)
        grouped[c.clinica_id].especialidades.add(c.especialidade.trim());
      if (c.operadora)
        grouped[c.clinica_id].operadoras.add(c.operadora.trim());
    });
    ...
    // AGORA: só entram no mapa clínicas com lat/lng válidos
    ...
    setClinicCount(listaFiltrada.length);
    setVisibleCount(20);

    if (listaFiltrada.length > 0) {
      setSelectedClinica(listaFiltrada[0]);
      setMapOpen(true);
      setTimeout(() => {
  ...
  const showClearButton =
    nomeClinica || especialidade || uf || cidade || searchPerformed;
      ...
      <div className="w-full max-w-6xl mt-10 px-4">
        <div className="bg-white border border-[#E4E6EA] rounded-2xl shadow-md p-8 space-y-6">
          ...
          <p className="text-lg text-[#5A6275] text-center mb-6">
            Busque por especialidade, convênio, localidade e nome da clinica.
          </p>
        ...
        {searchPerformed && (
          <div className="mt-10 space-y-10">
            <div className="flex justify-between items-center px-2">
              ...
              {(nomeClinica || especialidade || uf || cidade) && (
                <p className="text-md text-gray-600">
                  {clinicCount} clínica(s) encontrada(s)
                </p>
              )}
            </div>
            ...
            <div className="space-y-5">
              {loading && (
                <p className="text-lg animate-pulse text-center">
                  Carregando…
                </p>
              )}
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-[#5A6275] mt-2">
                        <strong className="text-[#0F1424]">
                          Especialidades:
                        </strong>{" "}
                        {c.especialidades.join(", ")}
                      </p>
                      <div className="mt-2 flex items-center gap-2">
                        <strong className="text-[#0F1424] text-sm">
                          Convênios:
                        </strong>
                        {c.operadoras.map((op) => (
                          <span
                            key={op}
                            className="text-xs font-semibold bg-gray-200 text-gray-700 px-2 py-1 rounded-full"
                          >
                            {op}
                          </span>
                        ))}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        // ensure the map focuses the clinic even if the same object is reused by the list
                        // try the direct global helper first, otherwise dispatch the custom event
                        setTimeout(() => {
                          try {
                            if (window && typeof window.__GEAP_focusClinic === "function") {
                              window.__GEAP_focusClinic(c);
                            } else {
                              window.dispatchEvent(new CustomEvent("geap:focusClinic", { detail: c }));
                            }
                          } catch (e) {
                            // ignore
                          }
